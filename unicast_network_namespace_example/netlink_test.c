...
static void netlink_test_recv_msg(struct sk_buff *skb)
{
    ...
    printk(KERN_INFO "netlink_test: netlink_test_recv_msg: Received %s\n", msg);
    ...
    printk(KERN_INFO "netlink_test_recv_msg: Received from pid %d, namespace %p, net_id: %d: %s\n",
           pid, net, net_id, msg);

    // get our data for this network namespace
    struct ns_data *data = net_generic(net, net_id);
    if (data == NULL || data->sk == NULL) {
      printk(KERN_ERR "netlink_test_recv_msg: data or socket is NULL\n");
      return;
    }
    ...
    if (!skb_out) {
      printk(KERN_ERR "netlink_test_recv_msg: Failed to allocate new skb\n");
      return;
    }
    ...
    if (res < 0)
      printk(KERN_INFO "netlink_test_recv_msg: Error while sending skb to user\n");
}
...
static int __net_init ns_netlink_test_init(struct net *net)
{
  ...
  if (!nl_sock) {
    printk(KERN_ALERT "ns_netlink_test_init: Error creating socket.\n");
    return -ENOMEM;
  }
...
{
  printk(KERN_INFO "netlink_test_init: Init module\n");
...
{
  printk(KERN_INFO "netlink_test_exit: Exit module\n");

  unregister_pernet_subsys(&audit_net_ops);
}
